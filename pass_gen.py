...
def user_pass_gen_config():
    ...
    user_config['gen_method']= input("""_____--------Simple_Password_Generator_0.1v--------_____
...
def string_distortion(user_config):
    gen_password = list(user_config['user_string'])
...
def mnemonics_generator(user_config):
    ...
    for i in range(0, len(user_config['user_string'])):
        while True:
            #todo find a faster and better API
            ...
            if word[0] == u_string[i]:
                main_string += word
                break
    main_string = string_distortion(user_config)
...
def main_user_pass_gen(user_config):
    if user_config['gen_method'] == '1': gen_password = mnemonics_generator(user_config)
        ...
        while True:
            if user_config['gen_method'] == '2': gen_password = string_distortion(user_config)
